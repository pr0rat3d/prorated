...
- **Two separate iOS locks, both in `codemagic.yaml`, both bite independently — every resubmission needs to check both:** (1) `agvtool new-version -all N` (build number) must be higher than *any* build number Apple has ever accepted, even from a different marketing version, or upload fails with `ENTITY_ERROR.ATTRIBUTE.INVALID.DUPLICATE`. (2) `agvtool new-marketing-version X.Y` — once that version string has been **approved** for public release (not just uploaded to TestFlight), Apple closes that "pre-release train" permanently; any further build under the same X.Y fails with `Invalid Pre-Release Train... is closed for new build submissions` + a matching `CFBundleShortVersionString` error, **regardless of the build number**. Bumping only one of the two when the other is also stale still fails — happened for real 2026-07-27 (1.0→1.1 bump fixed a build-number collision earlier in the cycle; then 1.1 itself got approved as build 37, so the next resubmission needed both build 39 *and* marketing version 1.2 before it would upload).
