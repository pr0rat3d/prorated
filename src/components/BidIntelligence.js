  ...
  return (
    <div>
      ...
      {isGoldPlus && status === "loaded" && report && (() => {
        ...
        const score = typeof report.score === "number" ? report.score.toFixed(1) : bidScore?.finalScore;
        const reviewCount = bidScore?.reviewCount ?? reviews?.length ?? 0;

        const copyReport = () => {
          const lines = [
            `🤖 BID INTELLIGENCE — ${addressLabel}`,
            `Generated by ProRated AI · ${(generatedAt || new Date()).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}`,
            "",
            `${sig.emoji} ${sig.label}`,
            `${conf.emoji} ${conf.label} · ${score}/5.0 · ${reviewCount} review${reviewCount !== 1 ? "s" : ""}`,
            "",
            "SITE ASSESSMENT",
            report.summary || "",
          ];
          if (Array.isArray(report.risks) && report.risks.length > 0) {
            lines.push("", "KEY RISKS", ...report.risks.map(r => `⚠️ ${r}`));
          }
          if (Array.isArray(report.positives) && report.positives.length > 0) {
            lines.push("", "KEY POSITIVES", ...report.positives.map(p => `✓ ${p}`));
          }
          if (report.recommendation) {
            lines.push("", "RECOMMENDATION", report.recommendation);
          }
          lines.push("", `Based on ${reviewCount} verified trade professional review${reviewCount !== 1 ? "s" : ""} · prorated.app`);
          ...
          <div style={cardBase}>
            ...
            <div style={{ background: "#fff", padding: "14px 16px" }}>
              ...
              <div style={{ fontSize: 11, color: BRAND.gray, marginTop: 10 }}>
                Based on {reviewCount} verified trade professional reviews
              </div>
              ...
              <button onClick={copyReport}
                style={{ width: "100%", background: copied ? "#16A34A" : BRAND.dark, color: "#fff", border: "none", padding: "10px", borderRadius: 10, fontSize: 12.5, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", marginTop: 12, transition: "background 0.2s" }}>
